...
public class UltimateMobileQuickbar : MonoBehaviour
{
	...
	void Update ()
	{
		...
		if( UnityEngine.InputSystem.EnhancedTouch.Touch.activeFingers.Count > 0 )
		{
			...
			for( int fingerId = 0; fingerId < UnityEngine.InputSystem.EnhancedTouch.Touch.activeFingers.Count; fingerId++ )
			{
				// If a finger id has been stored, and this finger id is not the same as the stored finger id, then continue.
				if( interactPointerId >= 0 && interactPointerId != UnityEngine.InputSystem.EnhancedTouch.Touch.activeFingers[ fingerId ].index )
					continue;

				Vector2 touchPosition = UnityEngine.InputSystem.EnhancedTouch.Touch.activeFingers[ fingerId ].screenPosition;
				bool touchBegan = UnityEngine.InputSystem.EnhancedTouch.Touch.activeFingers[ fingerId ].currentTouch.phase == UnityEngine.InputSystem.TouchPhase.Began;
				bool touchEnded = UnityEngine.InputSystem.EnhancedTouch.Touch.activeFingers[ fingerId ].currentTouch.phase == UnityEngine.InputSystem.TouchPhase.Ended;

				// Process the touch input.
				ProcessInput( touchPosition, touchBegan, touchEnded, fingerId );
			}
		}
		...
		if( UnityEngine.InputSystem.EnhancedTouch.Touch.activeFingers.Count == 0 )
		{
			...
			ProcessInput( mouse.position.ReadValue(), mouse.leftButton.wasPressedThisFrame, mouse.leftButton.wasReleasedThisFrame, 1 );
		}
#endif
#else
		// If there are touches on the screen...
	...
	void ResetQuickbar ()
	{
		// If the stored current button index is assigned and within range, call OnInputUp so that the button will be released.
		if( CurrentButtonIndex >= 0 && CurrentButtonIndex < QuickbarParents[ CurrentParentIndex ].QuickbarButtons.Count )
			QuickbarParents[ CurrentParentIndex ].QuickbarButtons[ CurrentButtonIndex ].OnInputUp();
	...
	/// <summary>
	/// This function is called by Unity when the device changes focus.
	/// </summary>
	void OnApplicationFocus ( bool focus )
	{
		if( !Application.isPlaying || !QuickbarActive || !focus )
			return;
	...
	public void UpdateQuickbarPositioning ()
	{
		...
		for( int i = 0; i < QuickbarParents.Count; i++ )
		{
			...
			for( int n = 0; n < QuickbarParents[ i ].QuickbarButtons.Count; n++ )
			{
				...
				if( QuickbarParents[ i ].QuickbarButtons[ n ].Registered )
					QuickbarParents[ i ].QuickbarButtons[ n ].OnInputUp();
